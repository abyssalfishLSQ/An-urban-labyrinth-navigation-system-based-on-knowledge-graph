<template>
  <div class="whole">
    ...
    <div class="fixed-button">
      <el-button type="primary" @click="confirmDialog" style="background-color: #728A7A; color: black;">查看3D模型
      </el-button>
    </div>
    <el-dialog
        title="场景说明"
...
export default {
  ...
  async mounted() {
    const nodeTypes = ['Province', 'City', 'Area', 'Place', 'POI', 'Time', 'Weather', 'Person', 'TripMode', 'Cause'];
    for (let i = 0; i < nodeTypes.length; i++) {
      const nodeType = nodeTypes[i];
      await this.fetchNodeDataByType(nodeType);
    }
    this.SearchKey = this.customNodes;
    await this.fetchRelationDataByType(''); // 进行关系数据的查询
  },
  methods: {
    async callAI() {
    ...
    confirmDialog() {
      window.location.href = 'https://app.graphxr.cn/p/65153843a79f13e55025aa4c/3D%E7%9F%A5%E8%AF%86%E5%9B%BE%E8%B0%B1%E7%BD%91%E7%AB%99';
    },
    onMouseDown(event) {
          ...
          .then(response => {
            ...
            if (Array.isArray(response.data.object)) {
              ...
              response.data.object.forEach(node => {
                const newNode = {
                  id: node.id, // 将id转换为字符串形式
                  label: nodeType, // 使用节点类型作为标签
                  properties: {
                    number: node[nodeType.toLowerCase() + 'Id'], // 增加索引以匹配你所需的数字格式
                    name: node[nodeType.toLowerCase() + 'Name'] // 使用属性名称动态获取节点名称
                  }
                };
    ...
    fetchRelationDataByType(placeName) {
      axios.post(`http://localhost:9093/Relation/findPWeather?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 格式化数据并添加到 customRelationships 数组中
              const formattedData = dataArray.map(item => ({
                source: item.place.id,
                sourcelabel: 'Place',
                target: item.weather.id,
                targetlabel: 'Weather',
                type: 'lost_weather',
              }));
              ...
              console.error('来自后端的lost_weather数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的lost_weather数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPPerson?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 格式化数据并添加到 customRelationships 数组中
              ...
              console.error('来自后端的lost_person数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的lost_person数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPTime?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 格式化数据并添加到 customRelationships 数组中
              ...
              console.error('来自后端的lost_time数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的lost_time数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPPOI?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 格式化数据并添加到 customRelationships 数组中
              ...
              console.error('来自后端的lost_POI数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的lost_POI数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPCause?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 格式化数据并添加到 customRelationships 数组中
              ...
              console.error('来自后端的lost_reason数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的lost_reason数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPTripMode`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 格式化数据并添加到 customRelationships 数组中
              const formattedData = dataArray.map(item => ({
                source: item.person.id,
                sourcelabel: 'Person',
                target: item.tripMode.id,
                targetlabel: 'TripMode',
                type: 'by',
              }));
              // 将格式化后的数据添加到 customRelationships 数组中
              this.customRelationships = this.customRelationships.concat(formattedData);
            } else {
              console.error('来自后端的by数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的by数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findACity`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 使用 filter 方法过滤掉包含 null 的数据
              const formattedData = dataArray
                  .filter(item => item.area !== null && item.city !== null)
                  .map(item => ({
                    source: item.area.id,
                    sourcelabel: 'Area',
                    target: item.city.id,
              ...
              console.error('来自后端的located_in(A-C)数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的located_in(A-C)数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findCProvince`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 使用 filter 方法过滤掉包含 null 的数据
              const formattedData = dataArray
                  .filter(item => item.province !== null && item.city !== null)
                  .map(item => ({
                    source: item.city.id,
                    sourcelabel: 'City',
                    target: item.province.id,
              ...
              console.error('来自后端的located_in(C-P)数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的located_in(C-P)数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findAProvince`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 使用 filter 方法过滤掉包含 null 的数据
              const formattedData = dataArray
                  .filter(item => item.province !== null && item.area !== null)
                  .map(item => ({
                    source: item.area.id,
                    sourcelabel: 'Area',
                    target: item.province.id,
              ...
              console.error('来自后端的located_in(A-P)数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的located_in(A-P)数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPProvince?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 使用 filter 方法过滤掉包含 null 的数据
              const formattedData = dataArray
                  .filter(item => item.province !== null && item.place !== null)
                  .map(item => ({
                    source: item.place.id,
                    sourcelabel: 'Place',
                    target: item.province.id,
              ...
              console.error('来自后端的located_in(P-P)数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的located_in(P-P)数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPCity?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 使用 filter 方法过滤掉包含 null 的数据
              const formattedData = dataArray
                  .filter(item => item.city !== null && item.place !== null)
                  .map(item => ({
                    source: item.place.id,
                    sourcelabel: 'Place',
                    target: item.city.id,
              ...
              console.error('来自后端的located_in(P-C)数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的located_in(P-C)数据错误:`, error);
          });
      axios.post(`http://localhost:9093/Relation/findPArea?placeName=${placeName}`)
          .then(response => {
            const dataArray = response.data.object;
            if (Array.isArray(dataArray)) {
              // 使用 filter 方法过滤掉包含 null 的数据
              const formattedData = dataArray
                  .filter(item => item.area !== null && item.place !== null)
                  .map(item => ({
                    source: item.place.id,
                    sourcelabel: 'Place',
                    target: item.area.id,
              ...
              console.error('来自后端的located_in(P-A)数据不是一个数组:', dataArray);
            }
          })
          .catch(error => {
            console.error(`来自后端的located_in(P-A)数据错误:`, error);
          });
    },
    showNodeInfo(node) {
      if (node && node.label) {
    ...
    getDataAndDraw() {
      ...
      node.append("circle")
          ...
          .attr("fill", d => {
            ...
            switch (d.label) {
              case 'Province':
                return d.properties.ProvinceName ? d.properties.switchActiveColor : '#3ca8cc';
              case 'City':
                return d.properties.CityName ? d.properties.switchActiveColor : '#c9b051';
              case 'Area':
                return d.properties.AreaName ? d.properties.switchActiveColor : '#49a964';
              case 'Place':
                return d.properties.PlaceName ? d.properties.switchActiveColor : '#fc7349';
              case 'POI':
                return d.properties.POIName ? d.properties.switchActiveColor : '#7fad7b';
              case 'Time':
                return d.properties.TimeName ? d.properties.switchActiveColor : '#6f95d0';
              case 'Weather':
                return d.properties.WeatherName ? d.properties.switchActiveColor : '#e8a75a';
              case 'Person':
                return d.properties.PersonName ? d.properties.switchActiveColor : '#c280c2';
              case 'TripMode':
                return d.properties.TripModeName ? d.properties.switchActiveColor : '#be7843';
              case 'Cause':
                return d.properties.CauseName ? d.properties.switchActiveColor : '#8377b6';
              default:
                return '#ccc'; // 默认颜色
            }
          })
