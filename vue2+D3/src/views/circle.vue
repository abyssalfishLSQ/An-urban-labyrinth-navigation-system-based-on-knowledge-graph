...
export default {
  ...
  methods: {
    searchData() {
      ...
      axios
          .post(`http://localhost:9093/Node/findPlacesByCityName?CityName=${this.searchQuery}`)
          .then((response) => {
            console.log('响应:', response.data);
            if (Array.isArray(response.data.object) && response.data.object.length > 0) {
              this.city = response.data.object.map((place) => {
                return {
    ...
    getData() {
      axios
          .post('http://localhost:9093/Node/findAllPlace')
          .then(response => {
            if (Array.isArray(response.data.object)) {
              this.locations = response.data.object.map(place => {
                return {
                  name: place.placeName,
                  query: place.placeName,
                  number: place.placeId
